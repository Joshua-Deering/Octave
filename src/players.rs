...
use crate::audio::{ShortTimeDftData, FreqData,  WindowFunction};
...
impl SignalPlayer {
    ...
    pub fn do_short_time_fourier_transform(&self, window_size: f32, overlap: f32, window_func: WindowFunction) -> ShortTimeDftData {
        let mut dft_data = vec![vec![]; self.channels];
        for i in 0..self.channels {
            dft_data[i] = do_short_time_fourier_transform(&self.samples[i], self.sample_rate, window_size, overlap, window_func);
        }
        let ch = dft_data.len() as u32;
        let dfts = dft_data[0].len() as u32;
        let freqs = dft_data[0][0].len() as u32;
        ShortTimeDftData::new(dft_data, window_func, overlap, ch, dfts, freqs, self.sample_rate)
    }
...
impl FilePlayer {
    pub fn new(file_path: String) -> Self {
        ...
        reader.seek(SeekFrom::Start(file_meta.chunks.get("data").unwrap().0)).unwrap();
        let end_pos = (file_meta.file_size / (file_meta.bit_depth / 8)) as usize;
        
        Self { file_meta, finished: false, reader, pos: 0, end_pos }
    }
    ...
    pub fn next_chunk(&mut self, data: &mut Data) {
        ...
        let data = read_data_interleaved_unchecked(&mut self.reader, &self.file_meta, dat_slice.len());
